...
WORKDIR /src
COPY ["AWSLambdaProcessorDotNet.csproj", "AWSLambdaProcessorDotNet/"]
RUN dotnet restore "AWSLambdaProcessorDotNet/AWSLambdaProcessorDotNet.csproj"

WORKDIR "/src/AWSLambdaProcessorDotNet"
COPY . .
RUN dotnet build "AWSLambdaProcessorDotNet.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AWSLambdaProcessorDotNet.csproj" \
...
Execute unit tests
```
cd "AWSLambdaProcessorDotNet/test/AWSLambdaProcessorDotNet.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "AWSLambdaProcessorDotNet/src/AWSLambdaProcessorDotNet"
